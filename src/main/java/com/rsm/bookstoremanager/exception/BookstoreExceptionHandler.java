package com.rsm.bookstoremanager.exception;
...
public class BookstoreExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException exception,
        ...
        exception.getBindingResult().getGlobalErrors().forEach(objectError -> errors
                .add("Object " + objectError.getObjectName() + " " + objectError.getDefaultMessage()));
    ...
    private ResponseEntity<Object> buildResponseEntity(HttpStatus httpStatus, String message, List<String> errors) {
        ApiError apiError = ApiError.builder().code(httpStatus.value()).status(httpStatus.getReasonPhrase())
                .message(message).errors(errors).timestamp(LocalDateTime.now()).build();
